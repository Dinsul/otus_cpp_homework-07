...
RadixTree::RadixTree() : _head(std::make_shared<RadixNode>("", false))
{}
...
void RadixTree::add(const std::string &text)
{
    ...
    {
        _head = std::make_shared<RadixNode>(text);
    }
    else
    {
        add(_head, text);
...
void RadixTree::add(std::shared_ptr<RadixNode> &node, const std::string &text)
{
    ...
    if (res.first != node->text.end())
    {
        ...
        node->children.emplace_back(std::make_shared<RadixNode>(nNode));
...
void RadixTree::pushChild(std::shared_ptr<RadixNode> &node, const std::string &text)
{
...
//        }
    ...
    node->children.emplace_back(std::make_shared<RadixNode>(text));
}
...
void RadixTree::remove(std::shared_ptr<RadixNode> &node, const std::string::const_iterator &begin, const std::string::const_iterator &end)
{
        ...
        for (auto child = node->children.begin(); child != node->children.end();  ++child)
        {
            if ((*child)->text[0] == res.second[0])
            {
                ...
                if (!(*child)->is_end)
                {
                    ...
                    {
                        auto subChildren = (*child)->children.back();

                        subChildren->text = (*child)->text + subChildren->text;
                        node->children.emplace_back(subChildren);
...
void RadixTree::printMinP(const std::shared_ptr<RadixNode> &node, std::string nickname, std::string minP)
{
...
void RadixTree::printTree(const std::shared_ptr<RadixNode> &node, std::string fill)
{
...
bool RadixTree::find(std::shared_ptr<RadixNode> &node, const std::string::const_iterator &begin, const std::string::const_iterator &end)
{
    auto res = std::mismatch(node->text.begin(), node->text.end(), begin, end);
...
RadixNode::RadixNode(const RadixNode &other) : text(other.text), is_end(other.is_end), children(other.children)
{}
